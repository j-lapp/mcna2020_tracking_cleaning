...
recoding_preliminary <- function(r, loop) {
  ...
  r$female_0_5_calc <- r$female_2_calc + r$female_3_5_calc
  r$male_0_5_calc <- r$male_2_calc + r$male_3_5_calc
...
recoding_msni <- function(r, loop) {
  ...
  r$fes <- r$food_exp / r$tot_expenditure
  r$fes_score <- ifelse(r$fes <= 0.5, 1, ifelse(r$fes <= 0.65, 2, ifelse(r$fes <= 0.75, 3, 4)))
  r$fes_more_50 <- ifelse(r$fes > 0.5, 1, 0)
  r$fs_class_int <- rowMeans(r[,c("fes_score", "cs_score")])
  r$fs_class <- rowMeans(r[,c("fcs_score", "fs_class_int")])
  ...
  r$g37 <- ifelse(r$how_much_debt > 505000, 1, 0)
  r$g38 <- ifelse(r$reasons_for_debt %in% c("basic_hh_expenditure", "education", "food", "health"), 1, 0)
  r$lsg_el <- round2(rowSums(r[,c("g44", "b1", "b2", "g37", "g38")]) / 5 * 3) +1
  ...
  r$msni <- apply(r, 1, function(x) {
    step_1 <- as.numeric(max(unique(as.vector(t(x[c("lsg_health", "lsg_protection", "lsg_snfi")])))))
...
######
#Recoding livelihodds
recoding_livelihoods <- function(r, loop) {
  r$expshare_nfi <- r$nfi_exp / r$tot_expenditure
  r$expshare_debt <- r$debt_exp / r$tot_expenditure
  r$expshare_food <- r$food_exp / r$tot_expenditure
  r$expshare_rent <- r$rent_exp / r$tot_expenditure
  r$expshare_water <- r$water_exp / r$tot_expenditure
  r$expshare_medical <- r$medical_exp / r$tot_expenditure
  r$expshare_electric <- r$electric_exp / r$tot_expenditure
  r$expshare_shelter <- r$shelter_exp / r$tot_expenditure
  r$expshare_education <- r$education_exp / r$tot_expenditure
  r$expshare_child_cloth <- r$child_cloth_exp / r$tot_expenditure
  r$expshare_adult_cloth <- r$adult_cloth_exp / r$tot_expenditure
  r$expshare_communication <- r$communication_exp / r$tot_expenditure
  r$expshare_transportation <- r$transportation_exp / r$tot_expenditure
