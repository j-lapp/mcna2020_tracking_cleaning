...
audit_results <- function(audit_dir_path, df_raw, enumerator_code, x_uuid, today){
  ...
  audit_joined <- audit_df %>% 
    left_join(dplyr::select(df_raw, enumerator_code, x_uuid, today), by = c("uuid"="_uuid"))
...
audit_report_wide <- function(audit_results, enumerator_code){
  ...
  report_wide <- report %>%
    dplyr::select(Enumerator_Unique_Id, node, seconds_mean) %>%
...
df_str_audit_all <- df_raw %>%
  left_join(dplyr::select(df_no_audit_files, x_uuid, start_end), by = c("_uuid"="_uuid"))

# Calculating time using start - end time for missing audit files
df_str_audit_all <- df_str_audit_all %>%
  left_join(dplyr::select(duration_df, x_uuid, duration_minutes), by = c("_uuid"="_uuid"))
...
# report <- audit_df %>% 
#   group_by(enumerator_id, node) %>%
#   summarise(seconds_mean = round(mean(time_seconds, na.rm = T),0),
#             seconds_max = max(time_seconds),
#             seconds_min = min(time_seconds),
#             seconds_med = median(time_seconds))
# 
# report_wide <- report %>% 
#   dplyr::select(enumerator_id, node, seconds_mean) %>% 
#   pivot_wider(names_from = enumerator_id, values_from = seconds_mean)
# 
# report_wide_2 <- report %>% 
#   dplyr::select(enumerator_id, node, seconds_mean) %>% 
#   pivot_wider(names_from = node, values_from = seconds_mean, names_repair = "unique")
